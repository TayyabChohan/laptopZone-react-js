...
const onAfterSaveCell = (row, cellName, cellValue) => {
  if (cellName == 'COST_PRICE') {
    const data = {
      ...
      barcode: row.BARCODE_NO == null ? row.REPAIRE_ID : row.BARCODE_NO,
      exempt: that.props.exempt
    }
...
class LineTypes extends Component {
  ...
  componentDidUpdate (prevProps, prevState) {
    if (prevState.line_type_id !== this.state.line_type_id) {
      const data = {
        barcode:
          this.props.row.BARCODE_NO !== null
            ? this.props.row.BARCODE_NO
            : this.props.row.REPAIRE_ID,
...
const line_Type = (cell, row) => {
  // if (row.LINE_TYPE == 'SR') {
  //   const Line_types = [{ value: 'SR', label: 'Service' }]
      ...
      buttons: [
        {
          ...
          onClick: () => {
            const data = {
              id: row.LZ_POS_DET_ID,
              user_id: localStorage.getItem('userId'),
              barcode: row.BARCODE_NO == null ? row.REPAIRE_ID : row.BARCODE_NO
            }
            const id = row.BARCODE_NO == null ? row.REPAIRE_ID : row.BARCODE_NO
            this.props.delete_pos_barcode_edit(id, this.props.exempt, data)
          }
        },
        {
          label: 'No'
