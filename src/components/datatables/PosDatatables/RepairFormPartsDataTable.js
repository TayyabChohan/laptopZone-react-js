...
const amount_paid = (cell, row) => {
  const valuee = Number(row.AMOUNT_PAID)
...
const onAfterSaveCell = (row, cellName, cellValue) => {
  if (cellName == 'COST_PRICE') {
    ...
    that.props.change_cost_price_repaire(data)
  }
  if (cellName == 'ADVANCE_PAYMENT') {
    const data = {
      advance_payment: row.ADVANCE_PAYMENT,
      barcode: row.BARCODE_NO
    }
    that.props.change_advance_payment_repaire(data)
  }
...
class LineTypes extends Component {
  ...
  componentDidUpdate (prevProps, prevState) {
    if (
      ...
      that.props.change_line_type_repaire(data)
    }
  }
  render () {
    const Line_types = [
...
var that = ''
class RepairFormPartsDataTable extends Component {
  constructor (props) {
    super(props)
    this.state = {
      button_status: false
  ...
  onClickDelete = (cell, row) => {
    // console.log(row)
    this.props.delete_pos_barcode_repaire(cell)
  }
  ...
  render () {
    const footerData = [
      [
        ...
        //       i++
        //     ) {
        //       sum += Number(tableData[i].AMOUNT_PAID)
        //     }
        //     return <strong>{'$ ' + sum.toFixed(2)}</strong>
        //   }
        // },
        {
          label: 'ADVANCE_PAYMENT',
          columnIndex: 8,
          align: 'center',
          formatter: tableData => {
            let sum = 0
            for (
              ...
              const value = tableData[i].ADVANCE_PAYMENT
    ...
    const options = {
      ...
      sizePerPageList: [
        ...
        {
          text: 'All',
          value: this.props.pos_repair_barcode_detail.length
        }
...
export default RepairFormPartsDataTable
