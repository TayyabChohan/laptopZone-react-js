  ...
  handleDelete = (cell, row) => {
    ...
    DBCall.then(result => {
      ...
      if (result.delete) {
        ...
        var boxes = that.state.trackingNoList;
...
export class DeleteBarcode extends Component {
  ...
  handleDelete = (cell, row) => {
    ...
    DBCall.then(result => {
      ...
      if (result.delete) {
        notify("success", result.message);
        var boxes = that.state.barcodeList;
        for (var i = 0; i < boxes.length; i++) {
...
class ReceiveShipment extends Component {
  ...
  handleTrackingNoSearch = e => {
    e.preventDefault();
    const { baseUrl, searchTrackingNo } = this.state;
    let userId = localStorage.getItem("userId");
    $.LoadingOverlay("show");
    let insertUrl =
      baseUrl + "/laptopzone/reactcontroller/c_receiveShipment/getTrackingNo";
    const DBCall = new Promise(function(resolve, reject) {
      $.ajax({
        url: insertUrl,
        dataType: "json",
    ...
    DBCall.then(result => {
      ...
      if (result.insert) {
        notify("success", result.message);
        this.setState({
          trackingNoList: result.receiveShipments,
          searchTrackingNo: ""
        });
      } else {
        notify("error", result.message);
  ...
  handleBarcodeSearch = e => {
    ...
    DBCall.then(result => {
      ...
      if (result.insert) {
        notify("success", result.message);
        this.setState({
          barcodeList: result.box_details,
  ...
  render() {
    const { trackingNoList, barcodeList } = this.state;
    const options = {
    ...
    return (
      <React.Fragment>
        ...
        <section className="content-header">
          <h1>
            Receive Shipment <small>Control Panel</small>
          </h1>
          <ol className="breadcrumb">
            ...
            <li className="active">
              <Link to="/receiveShipment">Receive</Link>
            </li>
          </ol>
        </section>
        <section className="content">
          <div className="row">
            <div className="col-md-12">
              <div className="box">
                <div className="box-header">
                  <h3 className="box-title">Receive Shipment Boxes</h3>
                </div>

                <div className="box-body">
                  <div className="col-sm-12">
                    <Link to="/receiveShipmentList" style={{ color: "white" }}>
                      <button className="btn btn-info">
                        Show Received Shipments
                      </button>
                    </Link>
                    <form onSubmit={this.handleTrackingNoSearch}>
                      <div className="input-group">
                        <input
                          className="form-control pl"
                          type="text"
                          value={this.state.searchTrackingNo}
                          id="searchTrackingNo"
                          name="searchTrackingNo"
                          onChange={this.handleChange}
                    ...
                    <div className="pricingTable">
                      <div className="price-value">
                        <span className="amount">
                          Total Boxes: {trackingNoList.length}
                        </span>
                      </div>
            ...
            >
              <div className="box">
                <div className="box-header">
                  <h3 className="box-title">Receive Shipment Barcodes</h3>
                </div>
                <div className="box-body">
                  <div className="col-sm-12">
                  ...
                  <div className="col-md-12">
                    <div className="pricingTable">
                      <div className="price-value">
                        <span className="amount">
                          Total Barcodes: {barcodeList.length}
                        </span>
                      </div>
                    </div>
                    <div className="pricing-content">
